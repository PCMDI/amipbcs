...
# PJD  6 Sep 2016 	- Updated for out-of-band update 1-1-0a (Peter Rensch - CSIRO)
...
set newDir=SST_1-1-0a ; ## UPDATE : PATH REQUIRES UPDATING ##
set oldDir=SST_NEW5 ; ## UPDATE : PATH REQUIRES UPDATING ##
set oi2path=/work/durack1/Shared/150219_AMIPForcingData/${newDir}/
######
...
# Extract only 'new' months, Here 12 months and 'time' index values: e.g 3,14 (April through March), 5,15 (June through April), 16,19 (May through August) ; ##
###### UPDATE : YEARS REQUIRE UPDATING ##
ncks -O -h -d time,16,19 MODEL.OI2.ice.mnly.201501-201608.unf.nc ICE.update.nc ; # Extract May 2016 to August 2016 ## UPDATE : YEARS IN FILENAMES REQUIRE UPDATING ##
ncks -O -h -d time,16,19 MODEL.OI2.sst.mnly.201501-201608.unf.nc SST.update.nc ; ## UPDATE : YEARS IN FILENAMES REQUIRE UPDATING ##
...
ncdump -v date ../${oldDir}/MODEL.SST.HAD187001-198110.OI198111-201604.nc ; ## UPDATE : YEARS IN FILENAMES REQUIRE UPDATING ##
...
# V1.0.1a - extend 201604 to 201608
ncrcat ../${oldDir}/MODEL.ICE.HAD187001-198110.OI198111-201604.nc ICE.update.nc MODEL.ICE.HAD187001-198110.OI198111-201608.nc ; ## UPDATE : YEARS IN FILENAMES REQUIRE UPDATING ##
ncrcat ../${oldDir}/MODEL.SST.HAD187001-198110.OI198111-201604.nc SST.update.nc MODEL.SST.HAD187001-198110.OI198111-201608.nc ; ## UPDATE : YEARS IN FILENAMES REQUIRE UPDATING ##
# Make sure updates went correctly
ncdump -v date MODEL.SST.HAD187001-198110.OI198111-201608.nc ; ## UPDATE : YEARS IN FILENAMES REQUIRE UPDATING ##
# Purge partial new files
rm -f ICE.update.nc ; # These may not purge do to a file handle being unreleased by ncrcat
rm -f MODEL.OI2.ice.mnly.201501-201608.unf.nc ; ## UPDATE : YEARS IN FILENAMES REQUIRE UPDATING ##
rm -f SST.update.nc
rm -f MODEL.OI2.sst.mnly.201501-201608.unf.nc ; ## UPDATE : YEARS IN FILENAMES REQUIRE UPDATING ##
######
