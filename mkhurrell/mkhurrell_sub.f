...
c               additional diagnostics are now returned (8/3/19)
c
c       
...
c     shown above, but the coefficients (aa's, bb's, and cc's) depend on 
...
c
c     set control on whether derivatives needed to compute jacobian 
c        will be computed through an analytical approximation or
c        by numerical approximation.  The first imethod-1 iterations
c        the analytical approximation will be invoked. 
c        If imethod < 1, then the numerical approach will be used for all
c         iterations
      imethod = 0
c
...
c            The time series can be broken into indpendent segments
c            at points where consecutive max or min values are found.
...
c
      if (jj .eq. 0) then
c
c        Begin simple cyclic treatment (i.e., time series considered to represent
c            a single period of a repeating cycle)
c
         nnn = 0
...
c *********************************************************************
c  
c       numerically estimate jacobian elements: aa, bb, cc
c       and compute the mean (avg) for the month, based on the 
c       the current 3 mid-month values (ssm, ss, and ssp)
c       delta should be a small fraction of the range of ss over all times
c
...
c              value exceeds tmax at central time and following time.
      ...
      elseif ((dxm .eq. 0.0) .and. (dxp .eq. 0.0)) then
c           value exceeds tmax at central time and both neigbors
         s = tlimit
      endif
c
      return
